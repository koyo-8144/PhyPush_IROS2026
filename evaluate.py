...
def main():
    ...
    # Pass ALL SIX bounds so 'domain' labels use the same seen/unseen ranges as
    # configs.py. Omitting the mu bounds silently falls back to the function
    # defaults and mislabels every domain if they differ.
    _, _, seq_len, df, _ = create_dataloaders(
        df, config['batch_size'], M_SEEN_MIN, M_SEEN_MAX, MU_SEEN_MIN, MU_SEEN_MAX
    )
